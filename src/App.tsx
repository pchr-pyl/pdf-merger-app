...
const App: React.FC = () => {
  ...
  const handleModeChange = (newMode: 'merge' | 'convert') => {
    setMode(newMode);
    setMergedUrl(null);
  };
  ...
  return (
    <div className="app-container">
      ...
      <header>
        ...
        >
          {mode === 'merge' 
            ...
            : 'แปลงหน้า PDF เป็นรูปภาพ PNG และดาวน์โหลดเป็นไฟล์ ZIP'}
        </motion.p>
      </header>
      ...
      {files.length > 0 && (
        ...
        >
          ...
          <button 
            ...
            onClick={mode === 'merge' ? handleMerge : handleConvert}
            disabled={(mode === 'merge' ? files.length < 2 : files.length < 1) || isMerging || isConverting}
          >
            {isMerging || isConverting ? (
              <>
                <Loader2 className="loading-spinner" size={18} />
                {isMerging ? 'กำลังรวมไฟล์...' : 'กำลังแปลงไฟล์...'}
              </>
            ) : (
              <>
                <Download size={18} />
                {mode === 'merge' ? `รวมไฟล์ PDF (${files.length})` : `แปลงเป็นรูปภาพ (${files.length})`}
              </>
            )}
          </button>
        </motion.div>
      ...
      {mergedUrl && !isMerging && !isConverting && (
        <motion.div 
        ...
        >
          ...
          <h3 style={{ marginBottom: '0.5rem' }}>
            {mode === 'merge' ? 'รวมไฟล์สำเร็จ!' : 'แปลงไฟล์สำเร็จ!'}
          </h3>
          ...
          {mode === 'merge' && mergedUrl !== 'conversion-success' && (
